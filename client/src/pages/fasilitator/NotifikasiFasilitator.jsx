...
const TIPE_CONFIG = {
  IZIN: {
    label: 'Izin',
    warna: '#f97316',       // orange-500
    warnaBg: '#fff7ed',     // orange-50
    warnaBorder: '#fed7aa', // orange-200
    warnaBadgeBg: '#ffedd5',
...
// Tentukan URL navigasi untuk fasilitator berdasarkan tipe dan id_referensi
function getNavigasiUrl(notif) {
  const { tipe_notifikasi, id_referensi, link_tujuan } = notif;
  // Selalu prioritaskan link_tujuan dari DB jika sudah diisi
  if (link_tujuan && link_tujuan.trim() !== '') return link_tujuan;

  if (tipe_notifikasi === 'IZIN') return '/fasilitator/validasi-izin';
  return null;
}

function getLabelTombol(tipe) {
  if (tipe === 'IZIN') return 'Lihat Pengajuan Izin';
...
// ─── Filter tabs ──────────────────────────────────────────────────────────────
// Pisahkan izin berdasarkan keyword di pesan (backend menyertakan nama jenis_izin)
const isIzinKegiatanLuar = (n) =>
  n.tipe_notifikasi === 'IZIN' &&
  (n.pesan?.toLowerCase().includes('kegiatan luar') ||
   n.judul?.toLowerCase().includes('kegiatan luar'));

const isIzinPulangKampung = (n) =>
  n.tipe_notifikasi === 'IZIN' &&
  (n.pesan?.toLowerCase().includes('pulang kampung') ||
   n.judul?.toLowerCase().includes('pulang kampung'));

const FILTERS = [
  { label: 'Semua',          fn: () => true },
  { label: 'Izin Kegiatan',  fn: isIzinKegiatanLuar },
  { label: 'Pulang Kampung', fn: isIzinPulangKampung },
  { label: 'Belum Dibaca',   fn: n => !n.status_baca },
];


// ─── Halaman Utama ────────────────────────────────────────────────────────────
function NotifikasiFasilitator() {
  ...
  // Navigasi dari tombol Lihat di dropdown
  ...
  const filteredList = notifList.filter(activeFilter.fn);
  ...
  return (
    <DashboardLayout menuItems={FASILITATOR_MENU}>
      <div className="page-enter page-content" style={{ maxWidth: '760px', margin: '0 auto' }}>
        ...
        {/* Header */}
        <div className="section-animate" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '20px' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '24px', fontWeight: '700', color: '#1e293b' }}>Kotak Masuk</h1>
        ...
        {/* Filter tabs */}
        <div className="section-animate" style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
          {FILTERS.map(f => (
            <button
              key={f.label}
              ...
              style={{
                ...
                cursor: 'pointer',
                transition: 'all 0.2s',
                background: filter === f.label ? '#1e293b' : '#fff',
                color:      filter === f.label ? '#fff'    : '#64748b',
                borderColor: filter === f.label ? '#1e293b' : '#e2e8f0',
              }}
            >
              {f.label}
              {f.label === 'Belum Dibaca' && unreadCount > 0 && (
                <span style={{
                  marginLeft: '6px',
                  background: '#ef4444',
                  color: '#fff',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {loading ? (
            ...
            <div style={{
              ...
              <div style={{ fontSize: '15px', fontWeight: '600' }}>Kotak Masuk Kosong</div>
